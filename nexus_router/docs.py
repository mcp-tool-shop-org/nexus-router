...
from .plugins import InspectionResult, inspect_adapter
...
class AdapterEntry:
    ...
    config: dict[str, Any] | None = None  # Minimal config for inspection
...
def _render_adapter_section(
    ...
    if entry.repo_url:
        ci_badge = (
            f"[![adapter-ci]({entry.repo_url}/actions/workflows/"
            f"adapter-ci.yml/badge.svg)]({entry.repo_url}/actions/"
            f"workflows/adapter-ci.yml)"
        )
...
def generate_adapter_docs(
    ...
    *,
    ...
    if include_header and adapters:
        ...
        for entry in adapters:
            try:
                ...
                line = f"| [{entry.package_name}](#{kind}-adapter) | {kind} | {caps} | {status} |"
    ...
    if include_footer:
        ...
        sections.append(
            f"_Generated by nexus-router v{__version__} • {adapters_ok} adapters documented_"
        )
        sections.append("")
